...
// Docker image (and tag) that provides the kb-service MCP knowledge service.
const IMAGE = process.env.KB_IMAGE || DEFAULT_IMAGE;
const MIN_PORT = 1111;
const MAX_PORT = 9999;
const CONTAINER_PORT = 1111;
const DEFAULT_BIND_ADDRESS = "127.0.0.1";
// Host address the container port binds to; loopback by default for safety.
const KB_BIND_ADDRESS = (process.env.KB_BIND_ADDRESS || DEFAULT_BIND_ADDRESS).trim();
// Explicit opt-in required to bind a non-loopback address (network exposure).
...
const MODEL_CACHE_ROOT = "/root/.cache/chroma";
let kbVolume = null;
let modelCacheVolume = null;
// Local embedding model used to vectorize notes and queries for dense retrieval.
const KB_EMBEDDING_MODEL = process.env.KB_EMBEDDING_MODEL || "all-MiniLM-L6-v2";
// Max tokens per indexed chunk; larger keeps more context but coarsens retrieval.
const KB_CHUNK_TOKENS = process.env.KB_CHUNK_TOKENS || "220";
// Default number of results returned by a wiki_search call.
const KB_TOP_K = process.env.KB_TOP_K || "8";
// Minimum relevance score a result must clear to be returned (0..1).
const KB_MIN_RELEVANCE = process.env.KB_MIN_RELEVANCE || "0.35";
// Max evidence anchors inspected per note when checking freshness/drift.
const KB_EVIDENCE_MAX_ANCHORS = process.env.KB_EVIDENCE_MAX_ANCHORS || "12";
// How many neighbouring chunks to merge around a hit to restore local context.
const KB_MERGE_ADJACENT_WINDOW = process.env.KB_MERGE_ADJACENT_WINDOW || "1";
// Seconds between filesystem scans that reindex out-of-band note edits.
const KB_WATCH_INTERVAL_SECONDS = process.env.KB_WATCH_INTERVAL_SECONDS || "15";
// Enable BM25 lexical + dense hybrid retrieval; "0" falls back to dense-only.
const KB_HYBRID_SEARCH = process.env.KB_HYBRID_SEARCH || "1";
// Max lexical (BM25) candidates fetched before fusion with dense results.
const KB_LEXICAL_CANDIDATES = process.env.KB_LEXICAL_CANDIDATES || "50";
// Reciprocal-rank-fusion constant; larger flattens the weight of top ranks.
const KB_RRF_K = process.env.KB_RRF_K || "60";
// Weight of the dense (vector) signal when fusing hybrid search ranks.
const KB_DENSE_WEIGHT = process.env.KB_DENSE_WEIGHT || "1.0";
// Weight of the lexical (BM25) signal when fusing hybrid search ranks.
const KB_LEXICAL_WEIGHT = process.env.KB_LEXICAL_WEIGHT || "1.0";
// Min normalized BM25 score for a lexical-only hit to enter the pool (0..1).
const KB_LEXICAL_MIN_SCORE = process.env.KB_LEXICAL_MIN_SCORE || "0.35";
// Ranking penalty applied to notes whose evidence changed since verification.
const KB_EVIDENCE_CHANGED_PENALTY = process.env.KB_EVIDENCE_CHANGED_PENALTY || "0.05";
// Enable the optional ONNX cross-encoder reranker; "0" disables it.
const KB_RERANKER = process.env.KB_RERANKER || "0";
// Host directory holding the reranker model.onnx + tokenizer.json (mounted read-only).
const KB_RERANKER_MODEL_PATH = process.env.KB_RERANKER_MODEL_PATH || "";
// Number of top fused candidates the cross-encoder reranks before trimming to top_k.
const KB_RERANKER_TOP_N = process.env.KB_RERANKER_TOP_N || "20";
// When set (default), search for a free host port if the preferred one is taken.
const FIND_FREE_PORT =
  (process.env.KB_FIND_FREE_PORT || "1").trim().toLowerCase() !== "0";
const REQUEST_TIMEOUT_MS = 2500;
const READY_TIMEOUT_MS = 8000;
// Max time to wait for the container to report healthy on first start (ms).
...
async function startContainer(port) {
  ...
  const rerankerEnabled = ["1", "true", "yes", "on"].includes(
    KB_RERANKER.trim().toLowerCase()
  );
  // When reranking is enabled with a host model directory, mount it read-only so
  // the container can load the cross-encoder without baking it into the image.
  const CONTAINER_RERANKER_PATH = "/opt/reranker";
  const mountReranker = rerankerEnabled && KB_RERANKER_MODEL_PATH.trim() !== "";
  const rerankerModelPathForContainer = mountReranker
    ? CONTAINER_RERANKER_PATH
  ...
  const dockerArgs = [
    ...
    "-e",
    `KB_LEXICAL_CANDIDATES=${KB_LEXICAL_CANDIDATES}`,
    "-e",
    `KB_RRF_K=${KB_RRF_K}`,
    "-e",
    `KB_DENSE_WEIGHT=${KB_DENSE_WEIGHT}`,
    "-e",
    `KB_LEXICAL_WEIGHT=${KB_LEXICAL_WEIGHT}`,
    "-e",
    `KB_LEXICAL_MIN_SCORE=${KB_LEXICAL_MIN_SCORE}`,
    "-e",
    `KB_EVIDENCE_CHANGED_PENALTY=${KB_EVIDENCE_CHANGED_PENALTY}`,
    "-e",
    `KB_RERANKER=${KB_RERANKER}`,
    "-e",
    `KB_RERANKER_MODEL_PATH=${rerankerModelPathForContainer}`,
    "-e",
    `KB_RERANKER_TOP_N=${KB_RERANKER_TOP_N}`,
    "-v",
    ...
    `${kbVolume}:${KB_ROOT}`,
    "-v",
    `${modelCacheVolume}:${MODEL_CACHE_ROOT}`,
  ];
  if (mountReranker) {
    dockerArgs.push(
      "-v",
      `${path.resolve(KB_RERANKER_MODEL_PATH)}:${CONTAINER_RERANKER_PATH}:ro`
    );
  }
