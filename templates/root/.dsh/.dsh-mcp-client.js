// Workspace-delivered fork of @deepseek-ai/dsh-mcp-client 0.1.1-rc.2.
// DSH's official bridge reserves serverName process-wide, while this lifecycle
// mounts one bridge through each agent.ctx. Tool registration is already
// agent-scoped, so this fork deliberately lets identical server names coexist
// across agents and relies on the scoped tools registry to reject duplicates
// within one agent. Keep the remaining bridge behavior aligned with upstream.
...
* tools. Identical `serverName` values may coexist across agent scopes so each
* active model sees stable names; the scoped tool registry still rejects a
* duplicate inside one agent.
*
