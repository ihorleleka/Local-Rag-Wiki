...
// Workspace-local launcher used by the DSH profile bundle. It resolves the
// managed wiki-kit directory so custom --agents-dir installs work without
// evaluating Node-specific globals in Cordis YAML expressions.
...
function main() {
  ...
  if (!agentsRoot) {
    throw new Error("Local-Rag-Wiki is not installed in this workspace. Run `npx github:ihorleleka/Local-Rag-Wiki install .` first.");
  }
  const runner = path.join(agentsRoot, RUNNER_FILE);
  if (!fs.existsSync(runner)) throw new Error(`Local-Rag-Wiki MCP runner is missing: ${runner}`);
